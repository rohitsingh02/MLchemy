...
if __name__ == "__main__":
    ...
    df = pd.read_csv("./data/ps_s1_e2/train.csv")
    test_df = pd.read_csv("./data/ps_s1_e2/test.csv")
    sub = pd.read_csv("./data/ps_s1_e2/sample_submission.csv")
    ...
    #         # lgb_params.random_state = self.config.seeds[0]
    #         model = LGBMRegressor(**params)
    #     elif model_name == 'cat':
    #         model = CatBoostRegressor(**params)
    #     elif model_name == 'xgb':
    #         if hasattr(self.config, 'monotone_constraints'):
    #             params['monotone_constraints'] = self.config.monotone_constraints.__dict__

    #         # xgb_params.random_state = self.config.seeds[0]
    ...
    num_cols = [feature for feature in df.columns if 'cont' in feature]
    drop_cols = ["id", "target"]
